...
public class BIT_HomePage_StepDef {
	...
	public void user_verifies_primary_menu_options(io.cucumber.datatable.DataTable dataTable)
	...
	public void user_verifies_all_head_titles(io.cucumber.datatable.DataTable dataTable) throws InterruptedException {
		...
		for (int i = 0; i < actualTopMenus.size(); i++) {
			actualTopMenus.add(actualTopMenus.remove(0).toLowerCase());
		}
	...
	public void user_verifies_all_icon_box_title(io.cucumber.datatable.DataTable dataTable)
			throws InterruptedException {
		Thread.sleep(4000);
		...
		System.out.println(actualTopMenus);
		System.out.println(expectedTopMenus);
		assertTrue(actualTopMenus.contains(expectedTopMenus));
	}

	@Then("User verifies all elementor-testimonial-contents")
	public void user_verifies_all_elementor_testimonial_contents(io.cucumber.datatable.DataTable dataTable)
			throws InterruptedException {
		Thread.sleep(4000);
	...
	public void user_verifies_all_icon_box_descriptions(io.cucumber.datatable.DataTable dataTable)
			throws InterruptedException {
		Thread.sleep(4000);
	...
	public void user_verifies_footer_sections(io.cucumber.datatable.DataTable dataTable) throws InterruptedException {
		...
		System.out.println(expectedTopMenus);
		for (int i = 0; i < actualTopMenus.size(); i++) {
			System.out.println(actualTopMenus.get(i) + " vs " + expectedTopMenus.get(i));
			assertTrue(actualTopMenus.get(i), actualTopMenus.get(i).contains(expectedTopMenus.get(i)));
		}
	}
